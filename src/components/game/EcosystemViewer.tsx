...
// Remap: stage 0→1(Seed), 1→2(Sprouting), 2→3(Growing), 3→4(Thriving), 4-5→5(Forest), 6→6(Guardian)
    ...
    <div className={wrapperClass}>
      <div className="relative overflow-hidden rounded-2xl">
        <svg viewBox="0 0 800 450" className="w-full h-full" preserveAspectRatio="xMidYMid slice">
          ...
          {/* Mountains backdrop for state 4+ */}
          {state >= 4 && (
            <g opacity="0.2">
          ...
          <AnimatePresence>
            {/* State 1: Seedling + raindrop + dust */}
            {state === 1 && (
            ...
            {/* State 2: Small plant + butterfly + flowers */}
            {state === 2 && (
            ...
            {/* State 4: Fuller tree replaces young */}
          ...
          {/* Fireflies for state 5+ */}
          {state >= 5 && <Fireflies count={state >= 6 ? 10 : 6} />}

          {/* Leaves drifting for state 3+ */}
          {state >= 3 && (
